...
class Det2DTransformer(nn.Module):
    def __init__(
        ...
        self.reference_points = nn.Linear(d_model, 10) #2)
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, MSDeformAttn):
                m._reset_parameters()
        xavier_uniform_(self.reference_points.weight.data[:2], gain=1.0)
        constant_(self.reference_points.bias.data[:2], 0.)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, attn_mask=None,noised_label=None, noised_bbox=None, noised_error=None):
        ...
        # decoder
        hs, inter_references,init_reference_out,reference_points_cat = self.decoder(
            tgt,               
            reference_points,
            memory,
            spatial_shapes,
            ...
            query_embed,
            mask_flatten,
            bs=bs,attn_mask=attn_mask,noised_label=noised_label, noised_bbox=noised_bbox,
            noised_error=noised_error)
        ...
        intermediate_output = {
            ...
            'reference_points_cat':reference_points_cat
        }
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,idx,
                ...
                self_attn_mask=None):
        if idx == 0:
            tgt = self.pos_embed_layer(reference_points_cat)
        elif query_pos is None:
            query_pos = self.pos_embed_layer(reference_points_cat)
        ...
        if self.training:
            tgt3 = torch.cat(tgt2[num_noise:].split(bs, dim=1), dim=0).transpose(0, 1)
            tgt4 = torch.stack(tgt2[:num_noise].split(bs, dim=1)).mean(dim=0).transpose(0, 1)
            tgt2 = torch.cat((tgt4, tgt3), dim=1)
        else:
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, bs=None, attn_mask=None,noised_label=None,
                noised_bbox=None, noised_error=None):
        ##########################################################################33
        if noised_label is not None:
            noised_gt_proposals = torch.cat(
                (
                    noised_bbox,  # B, 60, 6(C)
                    noised_error, # B, 60, 1(C)
                    noised_label  # B, 60, 3(C)
                ),
                dim=-1,
            )
            reference_points_cat = torch.cat(  # 60,2 gt
                (noised_gt_proposals,
                 reference_points),
                dim=1
            )
        else:
            reference_points_cat = reference_points

        reference_points = reference_points_cat[..., :2]
        init_reference_out = reference_points_cat[..., :2]
        ###############################################################################3
        output = None  # tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                ####################################33
                reference_class = self.class_embed[lid](output).sigmoid().detach()
                reference_error = self.depth_embed[lid](output)[:, :, 0:1].sigmoid().detach()
                reference_points_cat = torch.cat(
                    (
                        reference_points,
                        reference_error,
                        reference_class,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points),init_reference_out,reference_points_cat
