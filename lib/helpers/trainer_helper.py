...
class Trainer(object):
    ...
    def train(self):
        ...
        ema_min_bbox=ema_max_bbox= ema_min_depth= ema_max_depth=None
        for epoch in range(start_epoch, self.cfg['max_epoch']):
            ...
            ema_min_bbox,ema_max_bbox, ema_min_depth, ema_max_depth=self.train_one_epoch(epoch,ema_min_bbox,ema_max_bbox,ema_min_depth,ema_max_depth)
            self.epoch += 1
            ...
            if (self.epoch % self.cfg['save_frequency']) == 0:
                ...
                if self.tester is not None:
                    ...
                    if wandb.run is not None:
                        ...
                        val_dict = {'val/eval_mAP': cur_result, 'val/best_mAP': best_result, 'epoch': self.epoch}
                        wandb.log(val_dict)
    ...
    def train_one_epoch(self, epoch,ema_min_bbox,ema_max_bbox,ema_min_depth,ema_max_depth):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.train_loader):
            ...
            outputs,outputs_dn,dn_meta,ema_min_bbox,ema_max_bbox,ema_min_depth,ema_max_depth = self.model(inputs, calibs, targets, img_sizes,ema_min_bbox,ema_max_bbox,ema_min_depth,ema_max_depth, dn_args=dn_args)
            mask_dict=None
            #ipdb.set_trace()
            detr_losses_dict = self.detr_loss(outputs, targets, mask_dict,outputs_dn,dn_meta)
            ...
            if batch_idx % 30 == 0:
                ...
                print("")
                if wandb.run is not None:
                    wandb_metrics = {f"train/{k}": v for k, v in detr_losses_dict_log.items()}
            ...
            progress_bar.update()
        progress_bar.close()
        return ema_min_bbox,ema_max_bbox,ema_min_depth,ema_max_depth
